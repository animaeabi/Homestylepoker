...
const el = {
  ...
  voiceCallTitle: document.getElementById("voiceCallTitle"),
  voiceCallSubtitle: document.getElementById("voiceCallSubtitle"),
  voiceCallAnswer: document.getElementById("voiceCallAnswer"),
  voiceCallDismiss: document.getElementById("voiceCallDismiss"),
  ...
  chatVoiceToggle: document.getElementById("chatVoiceToggle"),
...
function getServerVoiceState() {
  ...
  const rawCallerId = (
    ...
    voice?.callStartedByPlayerId ||
    voice?.callStartedByGroupPlayerId ||
    null
  );
  const rawCallerName = voice?.call_started_by_name || voice?.callStartedByName || "";
  const callStartedAtRaw = voice?.call_started_at || voice?.callStartedAt || null;
  const callStartedAtMs = callStartedAtRaw ? Date.parse(callStartedAtRaw) : NaN;
  ...
  return {
    ...
    callStatus,
    callActive,
    callStartedByPlayerId: callActive ? rawCallerId : null,
    callStartedByName: callActive ? String(rawCallerName || seatName(rawCallerId)) : "",
    callStartedAt: callActive && Number.isFinite(callStartedAtMs) ? callStartedAtMs : null,
  };
}
...
function canControlTableCall() {
  const table = getTable();
  return Boolean(
    canUseVoice() &&
    table?.created_by_group_player_id &&
    state.identity?.groupPlayerId &&
    table.created_by_group_player_id === state.identity.groupPlayerId
  );
}
...
function renderVoiceUi() {
  ...
  const disabled = uiState === "disabled" || uiState === "locked" || uiState === "hidden";
...
async function ensureVoiceConnected({ unmute = false } = {}) {
  if (state.voiceConnected && state.voiceCall) {
    try { state.voiceCall.setLocalAudio(Boolean(unmute)); } catch {}
    state.voiceSpeaking = Boolean(unmute);
    renderVoiceUi();
    return state.voiceCall;
  }
...
async function disconnectVoice({ silent = false, destroy = false } = {}) {
  ...
  try {
    try { call.setLocalAudio(false); } catch {}
...
async function leaveTableVoiceCall({ silent = false } = {}) {
  await disconnectVoice({ silent, destroy: false });
  if (!silent) toast("Left table call");
}

async function startHostedVoiceCall() {
  if (!canControlTableCall()) {
    toast("Only the host can start the table call.", "error");
    return;
  }
  ...
  const seatToken = getSeatToken();
  if (!seatToken) {
    toast("Take a seat to start a call.", "error");
    return;
  }
  try {
    await ensureMicrophonePermissionForVoiceCall();
...
async function endHostedVoiceCall() {
  if (!canControlTableCall()) {
    toast("Only the host can end the table call.", "error");
    return;
  }
  const seatToken = getSeatToken();
  if (!seatToken) {
    toast("Take a seat to end the call.", "error");
    return;
  }
  try {
    await online.endVoiceCall({
      tableId: state.tableId,
      actorGroupPlayerId: state.identity.groupPlayerId,
      seatToken,
    });
    if (state.voiceConnected || state.voiceJoining) {
      await disconnectVoice({ silent: true, destroy: false });
    }
    toast("Table call ended", "success");
    queueRtRefresh();
  } catch (error) {
    toast(error?.message || "Could not end table call.", "error");
  }
}
...
async function onVoiceFabClick(event) {
  ...
  if (uiState === "connected") {
    await leaveTableVoiceCall();
    return;
  }
  if (uiState === "end_call") {
    await endHostedVoiceCall();
  }
}
...
function shouldClearSettledHandVisuals(hand = getLatestHand()) {
  ...
  return !isShowdownPresentationActive(hand);
}
...
const VOICE_MIN_GAP_MS = 1500;         // just enough to not fight the serialized speech queue; free tiers cover the volume, 429s still trigger backoff
...
function characterIdForPlayer(playerId) {
...
function updateVoiceToggleUi() {
  ...
  el.chatVoiceToggle.classList.toggle("active", on);
...
function initChatVoice() {
  ...
  chatVoice.enabled = stored === "1";
  updateVoiceToggleUi();
  // If it was left on, unlock audio on the player's first interaction this load
  // (browsers reset the gesture requirement each page load).
  if (chatVoice.enabled) {
    const onFirstGesture = () => { chatVoice.unlock(); document.removeEventListener("pointerdown", onFirstGesture); };
...
function maybeLaunchStreetRevealFx(hand = getLatestHand()) {
  ...
  const remainingMs = Math.max(remainingByScheduleMs, maxFinishDelayMs);
  anim.cleanupTimer = setTimeout(() => {
...
function renderBoard() {
  ...
  const players = getHandPlayers();
  const board = clearedSettledHand ? [] : (Array.isArray(hand?.board_cards) ? hand.board_cards : []);
  const contestedShowdown = !clearedSettledHand && isContestedShowdown(hand, players);
  const resultRevealReady = !clearedSettledHand && isShowdownResultReady(hand);
  const showdownHighlightData = !clearedSettledHand ? getShowdownHighlightData(hand, players) : {
    boardHighlights: new Set(),
    holeHighlightsBySeat: new Map(),
    leaderSeats: new Set(),
  };
  const payoutActive = !clearedSettledHand && Boolean(state.potPushAnimation?.handId === hand?.id && state.potPushAnimation?.launched);
  const clearedPot = !clearedSettledHand && Boolean(hand?.id && state.clearedPotHandId === hand.id);
  const potTotal = clearedSettledHand ? 0 : ((payoutActive || clearedPot) ? 0 : Number(hand?.pot_total || 0));
  const bigBlind = Math.max(0.01, Number(getTable()?.big_blind || 1));
  const potBb = potTotal / bigBlind;
  const allInMode = !clearedSettledHand && Boolean(
    hand && (
...
function renderSeats() {
  ...
  const mySeat = getMySeat();
  const showEquity = !clearedSettledHand && hand && isActionStreet(hand.state) && handPlayers.some(hp => !hp.folded && hp.all_in);
  ...
  const turnRemaining = !clearedSettledHand && hand && hand.action_seat ? getTurnClock(hand) : null;
  ...
  tableSeats.forEach((seat, idx) => {
    ...
    if (empty) {
      ...
      if (!clearedSettledHand && hand && hand.button_seat === seat.seat_no) {
      ...
      // SB/BB label
      if (!clearedSettledHand && hand && shouldShowBlindPositionLabel(hand, hp, seat.seat_no)) {
...
function renderMyHand() {
  ...
  if (!clearedSettledHand && hand && badgesEl) {
  ...
  const heroContributionLabel = getSeatContributionLabel({
    seat: mySeat,
    handPlayer: hp,
    hand: clearedSettledHand ? null : hand,
  });
...
async function submitTurnAction(label, actionType) {
  ...
  try {
    ...
    // Surface a payload error (e.g. missing seat token) instead of a silent no-op.
...
function bindEvents() {
  ...
  // starting a drag; the click flips character voices on/off.
  el.chatVoiceToggle?.addEventListener("pointerdown", (e) => e.stopPropagation());
  el.chatVoiceToggle?.addEventListener("click", (e) => {
    e.stopPropagation();
    const next = !chatVoice.enabled;
