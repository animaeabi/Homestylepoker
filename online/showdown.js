...
export function describeSevenCardHand(cardTokens, preferredTokens = []) {
  const normalized = (cardTokens || []).map(normalizedToken);
  if (normalized.length < 5) {
    return { classRank: null, className: "", label: "", tuple: [], winningCards: [] };
  }
