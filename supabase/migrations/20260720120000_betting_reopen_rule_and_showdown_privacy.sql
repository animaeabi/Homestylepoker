-- Tier 3 audit fixes (server side):
--
-- 1) Betting reopen rule (short all-in). online_submit_action reset has_acted
--    for every live player on ANY aggression — including an all-in smaller
--    than a full raise. Under no-limit rules a short all-in does NOT reopen
--    betting: players who already acted may only call or fold. The old code
--    let them (and bots, and the client UI) re-raise illegally.
--    Fix: online_hand_players.raise_locked_street marks players who had
--    already acted when a short all-in bumped the bet. The marker stores the
--    street name, so advancing to the next street unlocks automatically and
--    no changes to online_advance_hand are needed. A locked player's raise or
--    raising all-in is downgraded to a call (always legal at that point)
--    instead of erroring, so bots and stale clients cannot wedge the hand.
--
-- 2) Showdown privacy. online_get_hand_state_viewer returned EVERY player's
--    hole_cards to every viewer once state hit showdown/settled — including
--    folded (mucked) hands and fold-win pots where nobody had to show. The
--    client already hides those visually, but the cards were in the payload
--    (visible in DevTools). Now hole cards are only revealed for: the viewer's
--    own seat, players who manually showed, and un-folded players in a
--    CONTESTED showdown (2+ players un-folded).

alter table online_hand_players
  add column if not exists raise_locked_street text;

create or replace function public.online_get_hand_state_viewer(p_hand_id uuid, p_viewer_group_player_id uuid default null::uuid, p_viewer_seat_token text default null::text, p_since_seq bigint default null::bigint)
 returns jsonb
 language plpgsql
 security definer
...
begin
  ...
  -- Reveal only a CONTESTED showdown (2+ un-folded players had to table their
  -- hands). Fold-wins and mucked hands stay private unless manually shown.
  ...
  select coalesce(
    jsonb_agg(
      case
        ...
        when coalesce(hp.manually_shown, false) then to_jsonb(hp)
        when v_reveal_all and not hp.folded then to_jsonb(hp)
...
create or replace function public.online_submit_action(p_hand_id uuid, p_actor_group_player_id uuid, p_action_type text, p_amount numeric default null::numeric, p_client_action_id text default null::text, p_seat_token text default null::text)
...
begin
  ...
  -- Betting reopen rule: a short all-in (less than a full raise) does not
  -- reopen betting for players who already acted this street; they may only
  -- call or fold. raise_locked_street marks those players (set below when a
  -- short all-in bumps the bet) and is scoped to the street name so the next
  -- street unlocks automatically. Downgrade an illegal re-raise to a call
  -- instead of erroring so bots and stale clients cannot wedge the hand.
  if p_action_type in ('raise', 'all_in')
     and coalesce(v_hand_player.raise_locked_street, '') = v_hand.state
     and online_normalize_money(v_current_contribution + v_stack) > v_prev_bet
  then
    p_action_type := 'call';
  end if;
  ...
  if p_action_type in ('bet', 'raise', 'all_in') and v_new_street_contribution > v_prev_bet then
    ...
    if p_action_type = 'bet' or v_prev_bet <= 0 or v_is_full_raise then
      -- A full bet/raise (or the street's opening wager) reopens betting for
      -- everyone still able to act.
      update online_hand_players
      set
        has_acted = false,
        raise_locked_street = null
      where hand_id = p_hand_id
        and seat_no <> v_hand_player.seat_no
        and not folded
        and not all_in
      ...
      -- Short all-in: everyone still gets to respond to the new price, but
      -- players who had already acted this street may only call or fold.
      update online_hand_players
      set
        raise_locked_street = case when coalesce(has_acted, false) then v_hand.state else raise_locked_street end,
        has_acted = false
      where hand_id = p_hand_id
        and seat_no <> v_hand_player.seat_no
        and not folded
