-- CRITICAL (audit): when every player is all-in just from posting the blinds
-- (e.g. a heads-up endgame where both stacks are <= their blinds), the hand had
-- no actionable seat and online_start_hand set state='showdown' with an EMPTY
-- board. Nothing could then deal the board: online_advance_hand refuses on
-- 'showdown', and the settler throws on a non-5-card board — so the table wedged
-- permanently. Deal the full board (with burns, like a real runout) before
-- entering showdown so the hand settles normally.
create or replace function public.online_start_hand(p_table_id uuid, p_started_by_group_player_id uuid default null::uuid, p_host_seat_token text default null::text)
...
begin
  ...
  if v_action_seat is null then
    -- Everyone is all-in from posting the blinds; no betting can occur. Deal the
    -- full board (with burns, like a real runout) so the hand can be settled at
    -- showdown instead of stalling on an empty board the settler rejects.
    if coalesce(array_length(v_remaining, 1), 0) >= 8 then
      v_runout_board := jsonb_build_array(
        v_remaining[2], v_remaining[3], v_remaining[4],  -- flop (burn = index 1)
        v_remaining[6],                                   -- turn (burn = index 5)
        v_remaining[8]                                    -- river (burn = index 7)
      );
      v_remaining := coalesce(v_remaining[9:array_length(v_remaining, 1)], array[]::text[]);
    end if;
  end if;
  ...
  set
    ...
    -- The price to enter is the full big blind even when the BB posted short
    -- (all-in for less than a blind); the short blind's shortfall is reconciled
    -- by the side-pot math at showdown. Using the raw short post here would let
    -- the whole table limp in for less than a big blind.
    current_bet = online_normalize_money(greatest(coalesce(v_table.big_blind, 0), v_sb_post, v_bb_post)),
    min_raise = greatest(1, coalesce(v_table.big_blind, 1)),
    action_seat = v_action_seat,
    board_cards = coalesce(v_runout_board, board_cards),
  ...
  if v_hand.state = 'showdown' then
    if v_runout_board is not null then
      perform online_append_hand_event(
        v_hand.id,
        p_table_id,
        'street_dealt',
        p_started_by_group_player_id,
        jsonb_build_object('street', 'river', 'board_cards', v_runout_board, 'burned', true)
      );
    end if;
    perform online_append_hand_event(
      v_hand.id,
