// Texas Hold'em showdown evaluator + payout resolver for Edge runtime.
...
function toCard(token: string): Card {
...
function straightHighFromRanks(ranks: number[]): number | null {
...
function best5Rank(cards7: Card[]): number[] {
...
function computeSidePots(players: Array<{ seatNo: number; committed: number; folded: boolean }>) {
...
function splitCents(totalCents: number, seats: number[]) {
...
export function resolveShowdownPayouts({
  ...
  const payoutCents = new Map<number, number>();
